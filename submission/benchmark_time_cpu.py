...
def bench_one(method, T, B, H, D, device, dtype, warmup, iters, knobs):
    ...
    if method == 'linformer':
        ...
        ace = BatchedACE(D, Kb, L, M, device=device)
        state['ace'] = ace
    ...
    def step():
        if method == 'softmax':
            ...
            out = linear_attention_favorplus(Q,K,V, proj=make_favorplus_projections(Q.shape[1], 256, Q.shape[-1]))
        else:
...
def main():
    ...
    p.add_argument('--exponents', type=int, nargs='+',
                   default=[11, 13, 15, 17, 19, 20, 21, 22, 23, 24, 25, 26],
                   help='Sequence length exponents: T = 2^e')
    p.add_argument('--methods', nargs='+',
                   default=['race'],
                   choices=['race'])
    p.add_argument('--angular_exp', type=float, default=8.0)
    ...
    for T in lengths:
        ...
        for m in args.methods:
            try:
                ...
                print(f'  {m:9s}  median fwd + bwd time: {t_ms:8.2f} ms')
